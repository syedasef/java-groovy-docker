node('master')   {
      def dockerImageName= 'syedasif22/javadedockerapp_$JOB_NAME:$BUILD_NUMBER'
      stage('SCM Checkout'){
         git 'https://github.com/syedasef/java-groovy-docker.git'          
      }
      stage('Build'){
     ...
     stage('Build Docker Image'){         
      ...
      stage('Publish Docker Image'){
         withCredentials([string(credentialsId: 'dockerpwd', variable: 'dockerPWD')]) {
              sh "docker login -u syedasif22 -p ${dockerPWD}"
         }
